(function() {
  ...
  const canvas = document.getElementById('codex-rings-canvas');
  if (!canvas) return;
  ...
  function urgency(remaining) {
    return Math.min(Math.max((45 - remaining) / 45, 0), 1);
  }

  function rgba(c, alpha) {
    const a = alpha != null ? alpha : c.a;
  ...
  function draw() {
    ...
    var u = Math.max(urgency(pRem), urgency(sRem));

    drawHalo(u);
    if (usageData.primary) drawRing(OUTER_R, OUTER_LW, pRem, 'primary', u);
    if (usageData.secondary) drawRing(INNER_R, INNER_LW, sRem, 'secondary', urgency(sRem));
    drawTickMarks();
    drawOrbitingGlint();
  ...
  canvas.addEventListener('mousemove', function(e) {
    ...
    hovering = dist >= 80 && dist <= OUTER_R + 20;
  });
